...
class ExperimentSetting():
    def __init__(self,branch_probability_thresholds,df_names,number_of_estimators_list,fixed_params,
                 num_of_iterations=50,filter_approaches=['probability','number_of_samples','combination']):
        self.num_of_iterations=num_of_iterations
        self.branch_probability_thresholds=branch_probability_thresholds
    ...
    def run(self):
        self.experiments=[]
        for threshold in self.branch_probability_thresholds:
    ...
    def run_experiment(self,branch_probability_threshold,df,x_columns,y_column,hyper_parameters_dict):
        for i in range(self.num_of_iterations):
            ...
            start_temp = datetime.datetime.now()
            cs = ConjunctionSet(x_columns, df, rf,branch_probability_threshold,filter_approach)
            ...
            result_dict.update(self.new_model_measures(test_x,test_y,new_model_builder))
    ...
    def decision_tree_measures(self,X,Y,dt_model):
        ...
        return result_dict
    def new_model_measures(self,X,Y,new_model_builder):
        result_dict={}
        probas,depths=[],[]
        for inst in X:
            prob,depth=new_model_builder.predict_instance_probas(inst)
            probas.append(prob)
            depths.append(depth)
